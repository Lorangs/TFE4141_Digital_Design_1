	...
	if {[file exists "$from/"]} {
		...
		foreach file $files {
			...
			while {[file exists $targetFile]} {
				set targetNewFileName "${targetFileName}_copy_${copyNr}"
...
proc project_dir {project_name} {
	#disp auto [list $project_name [file normalize "../projects/$project_name"] [file normalize "./$project_name"]]
	if {[file exists [file normalize "../projects/"]]} {
	#	puts 1
		return [file normalize "../projects/$project_name"]
	} else {
	#	puts 2
		return [file normalize "./$project_name"]
	}
}
...
proc genProj {_xil_proj_name_ top_design top_design_testbench source_files sim_files repo_dir} {
	...
	# Create project
	create_project ${_xil_proj_name_} [project_dir $_xil_proj_name_] -part $implementation_part
